...
class NCSNRunnerConditional():
    ...
    def train(self):
        ...
        if self.args.resume_training:
            ...
            print(f'Pretrained model successfully loaded checkpoint.pt')
            ...
            for i, (X, y) in enumerate(dataloader):
                ...
                if step % self.config.training.snapshot_freq == 0:
                    ...
                    if self.config.training.snapshot_sampling:
                        ...
                        init_samples = torch.rand(self.config.sampling.batch_size*self.config.data.num_classes, self.config.data.channels,
                                                  self.config.data.image_size, self.config.data.image_size,
                                                  device=self.config.device)
                        labels = torch.arange(self.config.data.num_classes).repeat(
                            1, self.config.sampling.batch_size).squeeze().long().to(self.config.device)
                        ...
                        save_image(sample,
                                   ...
                                   nrow=self.config.data.num_classes)
    ...
    def sample(self):
        ...
        for idx in range(self.config.sampling.num_batches):
            # Sample x_T from uniform distribution
            init_samples = torch.rand(self.config.sampling.batch_size*self.config.data.num_classes, self.config.data.channels,
                                      self.config.data.image_size, self.config.data.image_size,
                                      device=self.config.device)
            labels = torch.arange(self.config.data.num_classes).repeat(
            ...
            save_image(sample, os.path.join(self.args.image_folder, 'image_grid_{}.png'.format(
                idx)), nrow=self.config.data.num_classes)
